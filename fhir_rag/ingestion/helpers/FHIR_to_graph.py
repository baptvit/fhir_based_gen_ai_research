...
def flat_fhir_to_json_str(flat_fhir, name, fhir_str, consumer_id):
    output = '{' + f'name: "{name}",'
...
def resource_to_node(resource, consumer_id):
    ...
    flat_resource = flat_fhir_to_json_str(flatten_fhir(resource), resource_name(resource), convert_json(resource), consumer_id)
